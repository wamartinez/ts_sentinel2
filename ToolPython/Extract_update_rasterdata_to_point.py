...
# --input D:\TESISMASTER\VECTOR\Training_data_ImageryST\IM_20150725\test_samples8.shp
# --raster D:\TESISMASTER\IMAGES\TO_PROCESS_10m\Images\IM_20150725
...
list_path_raster = [os.path.join(args.raster,w) for w in os.listdir(args.raster) if w.endswith('.jp2') or w.endswith('.tiff') or w.endswith('.tif') ]
...
list_names_raster = [w.split('.')[0].split("_")[-2] for w in os.listdir(args.raster) if w.endswith('.jp2') or w.endswith('.tiff') or w.endswith('.tif')]
